...
import { Plus, Send, CheckCircle, Clock, MapPin, AlertCircle, FileText, Sparkles } from 'lucide-react';
...
export default function CitizenPortal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success) {
        setSuccessMsg('Complaint reported successfully! Ashmora AI Agents are evaluating priority.');
        setTitle('');
        setDescription('');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          {complaints.length === 0 ? (
            ...
            complaints.map((c) => (
              <div key={c.id} className="glass-card p-5 rounded-2xl border border-slate-800 hover:border-cyan-500/40 transition">
                ...
                {c.ai_summary && (
                  <div className="mb-3 p-2.5 rounded-xl bg-slate-900/90 border border-cyan-900/50 text-xs text-cyan-300 flex items-start gap-2">
                    <Sparkles className="w-4 h-4 text-cyan-400 shrink-0 mt-0.5" />
                    <div>
                      <span className="font-semibold block text-[10px] uppercase text-cyan-400 font-mono">AI Verification Insight:</span>
                      {c.ai_summary}
                    </div>
                  </div>
                )}
